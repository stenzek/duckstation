...
{
  return QSize(std::max(static_cast<int>(std::floor(static_cast<qreal>(size.width()) / device_pixel_ratio)), 1),
               std::max(static_cast<int>(std::floor(static_cast<qreal>(size.height()) / device_pixel_ratio)), 1));
}
