                                           ...
                                           const Error& error)
{
  ...
  if (!result)
  {
    // TODO: Make async
    if (!cancelled)
      QtUtils::MessageBoxCritical(this, tr("Hash Calculation Failed"), QString::fromStdString(error.GetDescription()));
  ...
  {
    m_ui.tracks->item(static_cast<int>(i), 4)
      ->setText(QString::fromStdString(CDImageHasher::HashToString(track_hashes[i])));
  }
  ...
  for (size_t track = 0; track < track_hashes.size(); track++)
  {
    QTableWidgetItem* hash_text = m_ui.tracks->item(static_cast<int>(track), 4);
    QTableWidgetItem* status_text = m_ui.tracks->item(static_cast<int>(track), 5);
    QBrush brush;
