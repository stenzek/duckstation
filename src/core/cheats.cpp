...
public:
  ...
  bool GetLine(std::string_view* line)
  {
    ...
    size_t end_position = m_current_offset + 1;
...
static std::vector<std::string> FindPatchFilesOnDisk(const std::string_view serial, std::optional<GameHash> hash,
                                                     bool cheats);
...
static void EnumerateChtFiles(const std::string_view serial, std::optional<GameHash> hash, bool cheats, bool for_ui,
                              bool load_from_database, const F& f);
...
std::vector<std::string> Cheats::FindPatchFilesOnDisk(const std::string_view serial, std::optional<GameHash> hash,
                                                      bool cheats)
{
                               ...
                               bool load_from_database, const F& f)
{
  ...
  if (for_ui || !Achievements::IsHardcoreModeActive())
    disk_patch_files = FindPatchFilesOnDisk(serial, for_ui ? hash : std::nullopt, cheats);
                                             ...
                                             bool load_from_database, bool sort_by_name)
{
  ...
  EnumerateChtFiles(serial, hash, cheats, true, load_from_database,
...
void Cheats::ReloadCheats(bool reload_files, bool reload_enabled_list, bool verbose, bool verbose_if_changed)
{
  ...
  if (reload_files)
  {
    ...
    if (const std::string& serial = System::GetGameSerial(); !serial.empty())
    {
      ...
      {
        EnumerateChtFiles(serial, hash, false, false, true,
      ...
      {
        EnumerateChtFiles(serial, hash, true, false, cheatdb_is_enabled,
                          [](const std::string& filename, const std::string& file_contents, bool from_database) {
...
void Cheats::UpdateActiveCodes(bool reload_enabled_list, bool verbose, bool verbose_if_changed)
{
  ...
  if (!g_settings.disable_all_enhancements)
  {
    patch_count = EnableCheats(s_patch_codes, s_enabled_patches, "Patches");
    cheat_count = AreCheatsEnabled() ? EnableCheats(s_cheat_codes, s_enabled_cheats, "Cheats") : 0;
  }
                                                                              ...
                                                                              const std::string_view data, Error* error)
{
  ...
  while (reader.GetLine(&line))
  {
    // skip comments/empty lines
    std::string_view linev = StringUtil::StripWhitespace(line);
    if (linev.empty() || !StringUtil::IsHexDigit(line[0]))
      continue;
