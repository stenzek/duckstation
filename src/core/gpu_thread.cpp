...
void GPUThread::SyncGPUThread(bool spin)
{
  ...
  if (spin)
  {
    ...
    Timer::Value start_time = Timer::GetCurrentValue();
...
void GPUThread::UpdateSettingsOnThread(const Settings& old_settings)
{
  ...
  if (ImGuiManager::UpdateDebugWindowConfig() || (PostProcessing::DisplayChain.IsActive() && !System::IsPaused()))
    Internal::PresentFrame(false, 0);
}
...
{
  return s_state.gpu_thread.IsCallingThread();
}
