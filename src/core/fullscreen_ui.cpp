...
{
  if (GPUThread::HasGPUBackend() && s_pause_menu_was_open)
  {
    s_current_main_window = MainWindowType::PauseMenu;
    QueueResetFocus(FocusResetType::ViewChanged);
  }
...
{
  ClosePauseMenu();
  s_current_main_window = GPUThread::HasGPUBackend() ? MainWindowType::None : MainWindowType::Landing;
  UpdateRunIdleState();
  FixStateIfPaused();
}
...
void FullscreenUI::DrawSettingsWindow()
{
  ...
  const float bg_alpha =
    GPUThread::HasGPUBackend() ? (s_settings_page == SettingsPage::PostProcessing ? 0.50f : 0.90f) : 1.0f;
