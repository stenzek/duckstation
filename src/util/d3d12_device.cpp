...
void D3D12Device::SetPipeline(GPUPipeline* pipeline)
{
  ...
  if (GPUPipeline::Layout layout = m_current_pipeline->GetLayout(); m_current_pipeline_layout != layout)
  {
    ...
    m_dirty_flags |= DIRTY_FLAG_PIPELINE_LAYOUT | DIRTY_FLAG_CONSTANT_BUFFER | DIRTY_FLAG_TEXTURES |
                     DIRTY_FLAG_SAMPLERS | (IsUsingROVRootSignature() ? DIRTY_FLAG_RT_UAVS : 0);
  }
}
