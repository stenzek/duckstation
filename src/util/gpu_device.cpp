...
bool GPUDevice::ResizeMainSwapChain(u32 new_window_width, u32 new_window_height, float new_window_scale, Error* error)
{
  if (!m_main_swap_chain)
  {
    Error::SetStringView(error, "No main swap chain is created.");
    return false;
  }

  return m_main_swap_chain->ResizeBuffers(new_window_width, new_window_height, new_window_scale, error);
}
