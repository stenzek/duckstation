...
HTTPCache::CacheArchivePtr HTTPCache::GetCacheArchive(Error* load_error /* = nullptr */)
{
  std::unique_lock lock(s_locals.cache_mutex);
  if (s_locals.cache_archive.IsOpen()) [[likely]]
    return CacheArchivePtr(std::move(lock), &s_locals.cache_archive);

  if (s_locals.tried_initialize_cache_archive && !load_error)
    return {};
  ...
  if (!s_locals.cache_archive.OpenPath(cache_path, CACHE_VERSION, load_error ? load_error : &error))
  {
    ERROR_LOG("Failed to initialize HTTP cache: {}", (load_error ? load_error : &error)->GetDescription());
    return {};
  }
...
{
  const auto cache = GetCacheArchive(error);
  if (!cache) [[unlikely]]
