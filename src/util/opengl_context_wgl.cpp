...
void OpenGLContextWGL::DestroySurface(SurfaceHandle handle)
{
  ...
  if (wglGetCurrentDC() == static_cast<HDC>(handle))
    MakeCurrent(nullptr);
...
{
  if (!wglMakeCurrent(m_current_dc, nullptr))
    return false;
                                            ...
                                            Error* error)
{
  ...
  if (m_rc)
  {
    if (!wglMakeCurrent(hdc, make_current ? new_rc : nullptr))
    {
      Error::SetWin32(error, "wglMakeCurrent() failed: ", GetLastError());
